-- Update deep_copy_project function to handle origin_id and forked_at columns
CREATE OR REPLACE FUNCTION public.deep_copy_project(source_project_id bigint, target_project_id bigint, target_user_id uuid)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    LOOP
        -- Copy the training with origin tracking
        INSERT INTO trainings (
            title, tagline, description, image_url, preview_url,
            created_by, project_id, origin_id, forked_at
        )
        SELECT 
            title, tagline, description, image_url, preview_url,
            target_user_id, target_project_id, source_training_id, CURRENT_TIMESTAMP
        FROM trainings 
        ...
        -- Increment fork count for the source training
        UPDATE trainings SET fork_count = fork_count + 1 WHERE id = source_training_id;
    END LOOP;
    
    -- Copy module_characters relationships using the mappings (including new skills and traits columns)
    INSERT INTO modules_characters (
        module_id, character_id, ordinal, prompt, skills, traits
    )
    SELECT 
        mm.new_id,
