...
SELECT diag('Test 1: Member can create their own history record');
...
SELECT results_eq(
    ...
    WITH inserted AS (
        ...
        VALUES (100, tests.get_supabase_uid('test.member.2'), 'Test assessment')
        RETURNING id
    )
...
SELECT diag('Test 3: Member cannot view other members history');
SELECT results_eq(
    $$
    SELECT count(*)::bigint FROM history 
...
SELECT diag('Test 4: Member cannot create history for another user');
SELECT throws_ok(
    $$
    INSERT INTO history (module_id, user_id, assessment_text)
    VALUES (100, tests.get_supabase_uid('test.member.3'), 'Test assessment')
    $$,
