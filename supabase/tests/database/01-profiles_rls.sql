...
SELECT
  diag ('Test 1: RLS is disabled for public schema');
...
SELECT
  diag ('Test 2: RLS is enabled for profiles table');
...
SELECT
  diag ('Test 3: Members can only view their own profile');
...
SELECT
  diag ('Test 4: Non-admin users can update their own profile - change their organization_id');
...
SELECT
  diag ('Test 5: Non-admin users cannot update other profiles');
