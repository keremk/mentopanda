...
INSERT INTO
  trainings (id, title, organization_id, is_public)
VALUES
  (100, 'Private Training Org 2', 2, FALSE),
...
SELECT
  tests.create_supabase_user ('test.manage.2', 'manager', 2);

SELECT
  tests.create_supabase_user ('test.member.2', 'member', 2);
...
SELECT
  tests.authenticate_as ('test.member.2');
...
SELECT
  results_eq (
    $$
    SELECT title FROM trainings WHERE id = 100
    $$,
...
SELECT
  diag ('Test 2: Manager from org 2 can manage private training from org 2');

SELECT
  tests.authenticate_as ('test.manage.2');
...
SELECT
  diag ('Test 3: Manager from org 2 cannot manage private training from org 3');
...
SELECT
  results_eq (
    $$
    SELECT title FROM trainings WHERE id = 101
    $$,
