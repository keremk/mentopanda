...
SELECT diag('Test 3: Member can enroll in private training from their organization');
SELECT results_eq(
    $$
    WITH inserted AS (
        INSERT INTO enrollments (training_id, user_id)
...
SELECT diag('Test 4: Member cannot enroll in private training from another organization');
SELECT tests.authenticate_as('test.member.2');
