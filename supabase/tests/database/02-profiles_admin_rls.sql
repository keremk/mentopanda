...
SELECT
  tests.create_supabase_user ('test.admin.2', 'admin', 2);
...
SELECT
  diag ('Test 1: Admin users can update other profiles in their organization');

SELECT
  tests.authenticate_as ('test.admin.2');
...
SELECT
  diag ('Test 2: Admin users cannot update profiles in other organizations');
