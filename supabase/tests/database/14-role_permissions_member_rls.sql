...
SELECT
  tests.create_supabase_user ('test.member.100', 'member', 100);
...
SELECT
  diag ('Test 1: Authenticated member can view role permissions');

SELECT
  results_eq (
    $$
    SELECT count(*)::bigint FROM role_permissions 
    WHERE role = 'member'::public.user_role AND permission = 'enrollment.manage'::public.app_permission
    $$,
    ARRAY[1::bigint],
...
SELECT
  diag ('Test 2: Member cannot insert role permissions');

SELECT throws_ok(
    $$
    INSERT INTO role_permissions (role, permission)
    VALUES ('member'::public.user_role, 'training.manage'::public.app_permission)
    $$,
...
SELECT
  diag ('Test 3: Member cannot update role permissions');
...
SELECT
  diag ('Test 4: Member cannot delete role permissions');
