...
SELECT
  diag ('Test 1 Anon users can view public trainings');
SELECT
  results_eq (
    $$
    SELECT count(*) FROM trainings WHERE is_public = true AND id = 100
    $$,
    ARRAY[1::bigint],
...
SELECT
  diag ('Test 2 Anon users should not see private trainings');
SELECT
  results_eq (
    $$
    SELECT count(*) FROM trainings WHERE is_public = false AND id = 101
    $$,
    ARRAY[0::bigint],
