...
SELECT
  diag ('Test 1: Member from org 2 can only view private training from org 2');
...
SELECT
  results_eq (
    $$
    SELECT count(*) FROM trainings WHERE id = 100
    $$,
    ARRAY[1::bigint],
...
SELECT
  diag ('Test 2: Member from org 3 cannot view private training from org 2');
